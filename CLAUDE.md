...
`PreloadWindow/TaskListController/SwiftUIHostController.swift` is a generic `NSViewController` wrapper (`SwiftUIHostController<Content: View>`) for embedding any SwiftUI view inside the AppKit hierarchy. `MenuContentController` hosts two SwiftUI views side by side this way:
- `TaskGridView` (`PreloadWindow/TaskGridController/Views/`) — the grid pane. `MenuContentController` owns **two** `TaskGridViewModel` instances, each permanently bound to one repository: `taskGridViewModel` (`.preloadWebsites()`) for `.home`, `focusMusicViewModel` (`.focusMusic()`) for `.focusMusic`. `MenuContentController` tracks `currentMenuItem: HamburgerMenuItem` (`Models/HamburgerMenuItem.swift`); on `select(_:)` it swaps which view model backs the grid pane via `activeGridViewModel` + `taskGridController.updateRootView(_:)`, then reloads that view model through `currentMenuItem.repository`. Opening a card creates a `.browser` window; the `+` card saves a new item into whichever menu item is currently selected. Only Home (`.preloadWebsites()`) cards show a delete ("x") button — `TaskGridView`'s `onDelete` is passed only when `currentMenuItem == .home`; deleting confirms via `NSAlert` then filters the item out of `items` and rewrites the whole array back to the repository's path (same full-array-save pattern as add, not a targeted Firebase `removeValue()`).
- `TaskListView` (`PreloadWindow/TaskListController/Views/`) — the list pane, driven by a single `ScrollViewViewModel` injected from `MenuContentController`, permanently bound to `.scrollViewData()`. It is **not** menu-selection-switchable: `MenuContentController.loadListData()` only runs when `currentMenuItem == .home`, so selecting Focus Music swaps the grid pane and leaves the list pane exactly as it was.
