...
extension FirebaseJSONRepository {
    /// Treats "nothing saved at this path yet" as empty rather than throwing.
    private func loadOrEmpty<Item>() async throws -> [Item] where Model == [Item] {
        do {
            return try await load()
        } catch RealtimeDatabaseError.noData {
            return []
        }
    }
    ...
    func addElement<Item: Identifiable>(_ item: Item) async throws -> [Item] where Model == [Item] {
        let updated = try await loadOrEmpty() + [item]
    ...
    func deleteElement<Item: Identifiable>(id: Item.ID) async throws -> [Item] where Model == [Item] {
        let updated = try await loadOrEmpty().filter { $0.id != id }
        try await save(updated)
        return updated
    }

    /// No-op (returns the array unmodified) if no element matches `id`.
    @discardableResult
