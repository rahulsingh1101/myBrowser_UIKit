...
- `Models/HamburgerMenuItem.swift` — enum of hamburger-menu sections (`.home`, `.focusMusic`), each pairing a display title with its own `FirebaseJSONRepository<[ItemModel]>` (`repository`), used only for the grid pane. Adding a new section means adding a case here plus a matching factory on `FirebaseJSONRepository`; the shared `MenuContentViewModel` picks up the new repository automatically via `menuItem.repository`, no new view model instance needed. If the new section also needs its own list-pane data, give it a dedicated `ScrollViewViewModel` instance and repository in `MenuContentController`.
