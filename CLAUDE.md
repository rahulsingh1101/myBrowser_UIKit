...
- **`MainContainerController`** — the home window: a URL search field plus `HomeController` (a SwiftUI-hosted grid/list of preloaded sites). Submitting a URL asks `AppWindowFactory` for a `.browser` window.
...
- `FirebaseJSONRepository<Model: Codable>` — generic repository: reads from Firebase at a fixed path, falling back to a bundled JSON resource on failure; writes go straight to Firebase. New data types should add a `static func` factory extension here (see `.preloadWebsites()`, `.scrollViewData()`) rather than constructing the generic type inline at call sites.
